...
export { BaseThemeTypings } from '@chakra-ui/styled-system/dist/shared.types.js';
import { ThemeTypings as ThemeTypings$1 } from '@chakra-ui/styled-system/dist/theming.types.js';
...
declare type ThemeTypings = CustomThemeTypings extends BaseThemeTypings ? CustomThemeTypings : ThemeTypings$1;

declare type ResponsiveArray<T> = Array<T | null>;
declare type ResponsiveObject<T> = Partial<Record<ThemeTypings["breakpoints"] | string, T>>;
declare type ResponsiveValue<T> = T | ResponsiveArray<T> | ResponsiveObject<T>;
declare type Length = string | 0 | number;
declare type Union<T> = T | (string & {});
declare type Token<CSSType, ThemeKey = unknown> = ThemeKey extends keyof ThemeTypings ? ResponsiveValue<Union<CSSType | ThemeTypings[ThemeKey]>> : ResponsiveValue<CSSType>;
declare type CSSMap = Record<string, {
    value: string;
    var: string;
    varRef: string;
}>;
declare type Transform = (value: any, theme: CssTheme, styles?: Record<string, any>) => any;
declare type WithCSSVar<T> = T & {
    __cssVars: Record<string, any>;
    __cssMap: CSSMap;
    __breakpoints: AnalyzeBreakpointsReturn;
};
declare type CssTheme = WithCSSVar<{
...
declare type SemanticValue<Conditions extends string, Token extends string = string> = Union<Token> | Partial<Record<"default" | Conditions, Union<Token>>>;
...
declare type FlatToken = PlainToken | SemanticToken;
declare type FlatTokens = Record<string, FlatToken>;
declare type FlattenTokensParam = {
    tokens?: object;
...
declare type ThemeScale = typeof tokens[number] | "transition.duration" | "transition.property" | "transition.easing";
...
declare type Config$1 = Record<string, PropConfig | true>;
...
declare type Pseudos = typeof pseudoSelectors;
...
declare type ThemeThunk<T> = T | ((theme: Record<string, any>) => T);
...
declare type CSSWithMultiValues = {
...
declare type RecursiveCSSObject<D> = D & (D | RecursivePseudo<D> | RecursiveCSSSelector<D>);
declare type SystemStyleObject = RecursiveCSSObject<CSSWithMultiValues>;
/**
 * @deprecated use `SystemStyleObject` instead
 */
declare type CSSObject = SystemStyleObject & {};
interface FunctionCSSInterpolation {
    (theme: Record<string, any>): CSSObject;
}
declare type StyleObjectOrFn = SystemStyleObject | FunctionCSSInterpolation;
...
declare type StyleFunctionProps = {
    colorScheme: string;
...
declare type SystemStyleFunction = (props: StyleFunctionProps) => SystemStyleObject;
declare type SystemStyleInterpolation = SystemStyleObject | SystemStyleFunction;
...
declare type PartsStyleObject<Parts extends Anatomy = Anatomy> = Partial<Record<Parts["keys"][number], SystemStyleObject>>;
declare type PartsStyleFunction<Parts extends Anatomy = Anatomy> = (props: StyleFunctionProps) => PartsStyleObject<Parts>;
declare type PartsStyleInterpolation<Parts extends Anatomy = Anatomy> = PartsStyleObject<Parts> | PartsStyleFunction<Parts>;
...
declare type Theme = WithCSSVar<Record<string, any>>;
declare type Config = {
    parts?: string[];
...
declare type ValueType = ResponsiveValue<string | boolean>;
declare type Values = {
    theme: Theme;
    variant?: ValueType;
    size?: ValueType;
